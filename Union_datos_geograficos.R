...
keywords <- c("bbq", "balcon(es)?", "walk in closet", 
              "estudio", "garaje", "jacuzzi", "terraza(s)?",
              "remodelado", "remodelada", "parqueadero(s)?",
              "chimenea(s)?", "lavanderia(s)?", "gimnasio(s)?", "parque(s)?",
              "ascensor", "vigilancia", "patio", "vestier", "transmilenio",
              "sauna", "comunal", "duplex")
patterns <- paste0("\\b(", keywords, ")\\b")

# Crear variables binarias
for (i in seq_along(keywords)) {
  var_name <- paste0("has_", gsub(" ", "_", keywords[i]))
  train[[var_name]] <- as.integer(str_detect(tolower(train$description), regex(patterns[i], ignore_case = TRUE)))
}
...
train$text_combined <- paste(train$title, train$description, sep = " ")
...
# Capturar el estrato
train$estrato_raw <- str_extract(train$text_combined, pattern_estrato)
train$estrato_raw <- str_extract(train$estrato_raw, "(?i)(\\d|uno|dos|tres|cuatro|cinco|seis)")
train$estrato <- as.numeric(ifelse(train$estrato_raw %in% names(strato_dict), strato_dict[train$estrato_raw], train$estrato_raw))

# Regex para metros cuadrados
pattern_m2 <- "(?i)(\\d{2,4})\\s*(m2|m²|mts|metros cuadrados)"

# Capturar metros cuadrados
train$metros_cuadrados <- str_extract(train$text_combined, pattern_m2)
train$metros_cuadrados <- as.numeric(str_extract(train$metros_cuadrados, "\\d{2,4}"))
